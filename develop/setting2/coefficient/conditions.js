...
define(function (require) {
    ...
    ret.init = function (bubble) {
        ...
        mbegin.change(function () {
            var mbeginyear = mbegin.year - 0;
            var mbeginmonth = mbegin.month - 0;
            var mendyear = mend.year - 0;
            var mendmonth = mend.month - 0;
            //if (mbegin.year > mend.year || mbegin.year == mend.year && mbegin.month > mend.month) {
            if (mbeginyear > mendyear || mbeginyear == mendyear && mbeginmonth > mendmonth) {
                util.alert('时间选择有误，请重新选择！');
            }
        })
