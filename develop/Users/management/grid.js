...
define(function (require) {
    ...
    var storeConfig = {
        fields: [ 'USERID', 'USERNAME', 'ROLEID', 'ROLENAME', 'ROLECODE']
    }
    ...
    var gridConfig = {
        columns: {
            ...
            items: [{
                ...
                flex:0.6,
                renderer: function () {
                    return "<div class='image-buttons'><div class='btn-key'></div><div class='btn-delete'></div><div class='btn-edit'></div></div>";
                }
            }]
        },
    ...
    ret.init = function () {
        ...
        $('.grid-container').on('click', '.btn-delete', function (e) {
            ...
            if (confirm('确认要删除名为' + name + '的计算单元吗？')) {
        ...
        //行内按钮的点击事件  重置密码
        $('.grid-container').on('click', '.btn-key', function (e) {
            var record_id = $(this).parents('.x-grid-row').attr('data-recordid');
            var record = getRecordById(record_id);
            var id = record.USERID;
            if (typeof that.resetHandler === 'function') {
                that.resetHandler(record);
                ret.UserId = record.USERID;
            }
        })
    ...
    ret.reset = function (func) {
        this.resetHandler = func;
    }
