...
define(function (require) {
    ...
    ret.draw = function (data) {
        ...
        if (data && data[2]) {
            yMin = data[2].leftYMin;
            yMax = data[2].leftYMax;
            for (var j = 0; j < data[2].xAxis.length; j++) {
                selectedDisplays.push(data[2].xAxis[j]);
            }
            for (var i = 0; i < data[2].series.length; i++) {
                var a = data[2].series[i];
                ss.push({
                    name: a.name,
                    //markLine: {
                    //    data: [
