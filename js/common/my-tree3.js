...
MyTree.prototype.tree_ajax_success = function (data) {
    ...
    for (var i in list) {
        if (typeof list[i].ParentId != 'string') {
            ...
            //	firstLevels.push(list[i]);
            //} else if (list[i].ParentId == 'root') {
            //	firstLevels.push(list[i]);
        }
    }
    firstLevels.push(list[0]);
    ...
    for (var i = 0; i < firstLevels.length; i++) {
        var span = $("<span class='root' node_id='" + firstLevels[i].ModelBaseId + "'></span>");
        var iElement = $("<i class='icon-minus-sign icon-sign' node_id='" + firstLevels[i].ModelBaseId + "'></i>");
        span.append(iElement);
        span.append(firstLevels[i].ModelBaseName);
        span.attr('node_name', firstLevels[i].ModelBaseName);
        el.append(span);
        ...
        if (_.contains(this.current_open_node_ids, 'root')) {
            ...
            root_ul.addClass('opened');//总是展开
        }

        /* 渲染树状节点（递归） */
        var id_field = "ModelBaseId";
        var name_field = "ModelBaseName";
        var pid_field = 'ParentId';

        var root = { id: firstLevels[0].ModelBaseId, c: [], ul: $('.tree .elements'), level: 1, name: firstLevels[0].ModelBaseName };
        add_nodes(root);
    }
    ...
    var selectedNode = $(".tree span[node_id='" + this.current_select_id + "']");
    if (selectedNode.length > 0) {
        selectedNode.addClass('selected');
        $("#tree-current").html(selectedNode.attr('node_name'));
    } else {
        $("#tree-current").html('数据导航');
    }
